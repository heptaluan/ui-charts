...
export class CreateProjectModalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.initialState.type !== 'chart') {
      ...
      if (this.initialState.id) {
        ...
        this.getByConditionsTemplates(this.initialState.id, this.infoParent_id,
          this.infoLevel, this.infoOrder, this.infoPrice, this.infoType);
      } else {
        // 获取信息图
  ...
  handleScroll() {
    ...
    this.timer = setTimeout(() => {
      ...
      if (this.infoPage && top >= maxHeight) {
        ...
        // 如果是单个类别进来就走单个请求的，如果整个过来就请求整体
        if (this.initialState.id) {
          this.getByConditionsTemplates(this.initialState.id, this.infoParent_id, this.infoLevel, this.infoOrder,
            this.infoPrice, this.infoType, this.infoPage);
        } else {
          // 获取信息图
          this.getByConditionsTemplates(this.infoScene_id, this.infoParent_id, this.infoLevel, this.infoOrder,
  ...
  getByConditionsTemplates(scene_id, parent_id, level, order, price, type, page: number = 1) {
    this._http.get(`${this.infoConditionUrl}?scene_id=${scene_id}&parent_id=${parent_id}&level=${level}&order=${order}&price=${price}&type=${type}&pagenum=${page}`, this.httpOptions)
      .subscribe(data => {
        this.updateMasonryLayout = false;
        setTimeout(() => {
          this.updateMasonryLayout = true;
  ...
  previewInfo(e, item) {
    e.stopPropagation();
    // this.bsModalRef.hide();
    // this._router.navigate(['pages', 'templates', 'item'], { queryParams: { id: item.id } });
  ...
  loadMore() {
    if (this.loadTip === '加载更多...') {
      ...
      // this.getByConditionsTemplates(this.infoScene_id, this.infoParent_id,
      // this.infoLevel, this.infoOrder, this.infoPrice, this.infoType, this.infoPage);
    }
  }
