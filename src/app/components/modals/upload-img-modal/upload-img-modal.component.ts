...
export class UploadImgModalComponent implements OnInit, AfterViewInit {
  ...
  handleType(type: string) {
    ...
    this.desc = type === 'logo' ? '建议上传不大于50K的透明底PNG格式文件，参考尺寸：100px * 40px' : '建议上传不大于50K的透明底PNG格式文件';
  }
  ...
  delImg(id) {
    ...
    this._http.delete(`${this.url}/${id}`, options)
      .subscribe(() => {
  ...
  getUserUploadImgs(isDel = false) {
    this._http.get(`${this.url}?option=${this.type}`, this.httpOptions)
      .subscribe(res => {
        const imgs = res['data'];
  ...
  updateSetting() {
    ...
    if (this.selectImgUrl !== this.block.props.logoDisplay.imgUrl) {
      this._http.put(`${this.url}/${this.selectId}/`, { 'option': this.type }, this.httpOptions)
        .subscribe(() => {
          this.updateCurBlock();
        })
    } else {
  ...
  private resizeImages(img: any, type: string, size: number): string {
    ...
    return canvas.toDataURL(type, .8)
  }
