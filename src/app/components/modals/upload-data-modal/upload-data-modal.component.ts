...
export class UploadDataModalComponent implements OnInit {
  ...
  // 获取 id 对应的数据
  getOneData() {
    this._http.get(`${this.uploadUrl}${this.dataId}/`, this.httpOptions).subscribe((res) => {
      if (res) {
  ...
  uploadExcelAndCsv() {
    ...
    if (colNumArr.length) {
      ...
      newData[0] = _.map(newData[0], (item) => {
  ...
  chartTemplatesSort() {
    ...
    // obj-type-value, tree-value, sankey
      ...
      const filterArr = newChartFilterTemplates.reduce((acc, item) => {
        if (
          item.templateSwitch === 'obj-type-value' ||
          item.templateSwitch === 'tree-value' ||
          item.templateSwitch === 'sankey'
        ) {
          acc.unshift(item)
        } else {
          acc.push(item)
  ...
  oneStepButtonGroupHandle(flag?) {
    ...
    if (this.isUploadFile) {
      if (this.dataId) {
        this._http
          .put(`${this.uploadUrl}${this.dataId}/`, { data: this.data }, { withCredentials: true })
          .pipe(
            catchError((err) => {
        ...
        this._http
          .post(this.uploadUrl, data, { withCredentials: true })
          .pipe(
            catchError((err) => {
  ...
  selectChartTemplateHandle(item) {
    ...
    // tree-value、obj-type-value、sankey
      ...
      block.templateSwitch === 'tree-value' ||
      block.templateSwitch === 'obj-type-value' ||
      block.templateSwitch === 'sankey'
    ) {
      if (block.props.map[0].length <= 1) {
  ...
  fourStepButtonGroupBackHandle() {
    ...
    if (this.isVpl === 'None' && this.totalLength >= 10) {
      ...
      this._http
        .post(`${this._api.getOldUrl()}/vis/dychart/fork/template/${this.currentBlock.templateId}`, this.currentBlock, {
        ...
        .subscribe((res) => {
          if (res['resultCode'] === 1000) {
            ...
            this._store.dispatch(
              new ProjectActions.ConfigChartProjectAction(res['data']['id'], {
            ...
            tempPage.location.href =
              window.location.href.split('#')[0] + '#/pages/workspace?project=' + res['data'].id + '&type=chart'
            // 获取全部项目列表
            this._store.dispatch(new ProjectActions.GetAllProjectListAction())
          }
        })
