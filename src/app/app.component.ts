...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // 获取项目长度
    this.allProjectScription.add(this._store.select(fromRoot.getAllProjectList).subscribe(list => {
  ...
  ngAfterViewInit() {
    ...
    this.getUserInfoSubscription.add(this._store.select(fromRoot.getUserInfo).filter(user => !!user).subscribe(user => {
      ...
      if (window.self === window.top && !(/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent) || /(Android)/i.test(navigator.userAgent))) {
        // 版本更新提示
        if (this.userName) {
  ...
  createProject(type) {
    ...
    this._http.post<ProjectModels.ProjectInfo>(createProjectUrl, payload, { withCredentials: true }).subscribe(res => {
  ...
  exportZip() {
    ...
    if (vip && vip.startsWith('eip')) {
      ...
      if (shareUrl && projectType === 'chart') {
        getUrl = `https://dycharts.com/chartzipdownload?id=${shareUrl}&type=chart&hideToolbar=${this.hideToolbar}&fonts=${encodeURI(fonts)}`
        ...
        getUrl = `https://dycharts.com/chartzipdownload?id=${shareUrl}&hideToolbar=${this.hideToolbar}&fonts=${encodeURI(fonts)}`
      }
  ...
  sendZip() {
    ...
    if (vip && vip.startsWith('eip')) {
      ...
      let getUrl = ''
      if (shareUrl && projectType === 'chart') {
        getUrl = `https://dycharts.com/partner/eip_export_notify?id=${shareUrl}&type=chart&hideToolbar=${this.hideToolbar}&fonts=${encodeURI(fonts)}`
        ...
        getUrl = `https://dycharts.com/partner/eip_export_notify?id=${shareUrl}&hideToolbar=${this.hideToolbar}&fonts=${encodeURI(fonts)}`
      }
      const xhr = new XMLHttpRequest()
      const that = this
      xhr.open('get', getUrl, true)
  ...
  updateTip(time:number) {
    const historyEdition = ['0809', '1018', '1031', '1220', '0119', '0201', '0215', '0222', '0229', '0307', '0314', '0515', '0605', '0612', '0706', '1016', '1030', '1113', '1204', '20201218', '20201231', '20210115', '20210128'];
    ...
    if ((!updateTip || historyEdition.indexOf(updateTip) > -1 || (updateTip && updateTip.length === 4)) 
