...
export class PlanComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this._http.get(this._api.getUserPermissionConsume(), this.httpOptions)
      .subscribe(res => {
        const data = res['data'];
        this.usedPermissionConsumeArr.push({
          ...
          remained: data['max_projects_num'] === null ? '无限' : Number(data['max_projects_num'] - data['projects_count']) > 0 ? Number(data['max_projects_num'] - data['projects_count']) : 0,
          unit: '个'
        })
        this.usedPermissionConsumeArr.push({
          title: '图表下载量',
          ...
          remained: data['max_export_num'] === null ? '无限' : Number(data['max_export_num'] - data['daily_export_count']) > 0 ? Number(data['max_export_num'] - data['daily_export_count']) : 0,
          unit: '次'
        })
        this.usedPermissionConsumeArr.push({
          title: '上传数据存储空间 ',
          ...
          remained: data['max_upload_data_num'] === null ? '无限' : Number(data['max_upload_data_num'] - data['uploaded_data_count']) > 0 ? Number(data['max_upload_data_num'] - data['uploaded_data_count']) : 0,
          unit: '个'
        })
        this.usedPermissionConsumeArr.push({
          title: '图片素材存储空间',
          ...
          remained: data['img_upload_capacity_str'] === null ? '无限' : Math.round(Number(data['img_upload_capacity_str'] - data['img_upload_consumed_space']) * 100) / 100,
          unit: 'M'
        })
      })
    ...
    this.billListSubscribe = this.billList$.subscribe(info => {
      if (info) {
        ...
        const obj = { time: info['vip2_time'].split('：')[1], pro_name: '高级会员', price: price, invate_code: '无邀请码', total_time: timeLength };
        const obj1 = { time: info['vip1_time'].split('：')[1], pro_name: '会员', price: price, invate_code: '无邀请码', total_time: timeLength };
        const obj2 = { time: info['eip1_time'].split('：')[1], pro_name: '企业会员', price: price, invate_code: '无邀请码', total_time: timeLength };
        const obj3 = { time: info['eip2_time'].split('：')[1], pro_name: '高级企业会员', price: price, invate_code: '无邀请码', total_time: timeLength };
        this.listTop = [];
        ...
        this.listTop.unshift({ pro_name: '会员类型', time: '时间段', invate_code: '邀请码', price: '金额', total_time: '会员时长' });
      }
    });
