...
export class ConfigurationComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    router.events.filter(event => event instanceof NavigationEnd).subscribe((info: NavigationEnd) => {
  ...
  activeMenu(url) {
    ...
    if (!this.menuItems || !this.menuItems.nativeElement.children || this.menuItems.nativeElement.children.length === 0) {
      return;
    }
