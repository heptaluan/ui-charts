...
export class HomeSidebarComponent implements OnInit {
  ...
  // loginSidebar = {
  //   'height': innerHeight - 60 - 30 - 50 + 'px'
  // };
  ...
  loginUrl = `${this._api.getHost()}/auth/signin.html?redirect=` + encodeURIComponent(window.location.href.split('#')[0] + window.location.hash);
  registerUrl = `${this._api.getHost()}/auth/signin.html?redirect=`+ encodeURIComponent(window.location.href.split('#')[0] + window.location.hash);
  ...
  getUserLogin() {
    if (this._httpService.code === 2007) {
      location.href = `${this._api.getHost()}/auth/signin.html?redirect=`+ encodeURIComponent(window.location.href.split('#')[0] + window.location.hash);
    }
  }
