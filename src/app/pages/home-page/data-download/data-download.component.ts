...
export class DataDownloadComponent implements OnInit {
  ...
  ngOnInit() {
    // 百度统计
    // window['_hmt'].push(['_trackPageview', '/appv2/#/pages/home/datadownload']);
  ...
  search(title, page: number = 1) {
    ...
    this._http.get(`${this._api.getDownloadList()}?type=${this.dataType}&title=${title}&page=${page}`, this.httpOptions)
      .subscribe(data => {
  ...
  getDownLoadData(type, page: number = 1) {
    this.titleSearch = '';
    if (type === 'xlsx') {
      this._http.get(`${this.downloadUrl}?page=${page}&type=xlsx`, this.httpOptions)
        .subscribe(data => {
      ...
      this._http.get(`${this.downloadUrl}?page=${page}&type=pdf`, this.httpOptions)
        .subscribe(data => {
          this.dataReportTemplates = data['data']['results'];
  ...
  allDelete() {
    ...
    this._modalService.onHidden.pipe(take(1)).subscribe((r: string) => {
      if (this.bsModalRef.content.confirmFlag) {
        this._http.post(`${this.downloadUrl}`, { 'action': 'DELETE', 'data': this.listIds }, this.httpOptions)
          .pipe(
            catchError(err => {
