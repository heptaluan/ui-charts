...
import { InitializeProjectComponent, ProjectSizeLimitComponent, UpgradeMemberComponent} from '../../../components/modals';
...
export class CreateGraphicsComponent implements OnInit, OnDestroy {
  ...
  constructor(private _store: Store<fromRoot.State>,
    ...
    this.mySubscription.add(this._store.select(fromRoot.getProjectList).subscribe(list => {
      if (this._vipService.isVip()) {
        this.limited = list.length >= 50;
      } else {
        this.limited = list.length >= 10;
      }
    })).add(this._store.select(fromRoot.getProjectCaseSearch).pipe(take(1)).subscribe(search => this.initSearch = search));
  }

  ngOnInit() {
    this._store.dispatch(new TemplateActions.SetProjectCurrentTemplateTypeAction(this.type.tag));
    this.mySubscription.add(this.templates$.subscribe(templates => {
      // 避免重复请求模板
