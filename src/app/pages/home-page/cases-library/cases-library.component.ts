...
export class CasesLibraryComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.mySubscription
    .add(this._store.select(fromRoot.getProjectCaseList).subscribe(list => {
      this.cases = list;
    }))
    .add(this._store.select(fromRoot.getProjectCaseSortType)
      .pipe(combineLatest(this._store.select(fromRoot.getProjectCaseCurrentPageNum), this._store.select(fromRoot.getProjectCaseSearch)))
      .subscribe(data => {
        this.sort = data[0];
        this.currentPage = data[1];
        this.search = data[2];
        this._store.dispatch(new CaseActions.GetProjectCaseListAction({
      ...
      .add(this._store.select(fromRoot.getProjectCaseTotal).subscribe(total => {
        this.total = total;
      }));
  }

  ngOnInit() {
    this.mySubscription.add(this._router.events.filter(event => event instanceof NavigationStart).subscribe((route: NavigationStart) => {
