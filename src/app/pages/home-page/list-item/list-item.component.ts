...
import { PublishShareComponent, RenameComponent, UpgradeMemberComponent, ContactUsModalComponent, UploadDataModalComponent } from '../../../components/modals';
...
export class ListItemComponent implements OnInit {
  ...
  rename(item) {
    ...
    this._modalService.onHidden.pipe(take(1)).subscribe((r: string) => {
      if (this.bsModalRef.content.confirmFlag) {
        ...
        this._http.put(`${this._api.getUploadList()}${item.id_str}/`, { 'title': renameTitle }, this.httpOptions)
          .pipe(
            catchError(err => {
  ...
  createJumpToEditUrl(dataId) {
    ...
    this.editDataScription.add(this._store.select(fromRoot.getForkTemplateProjectId).subscribe(id => {
      if (id) {
        // 清空描述
        this._store.dispatch(new ProjectActions.ConfigChartProjectAction(id, { action: 'set_description', description: '' }));
        this._router.navigate(['pages', 'workspace'], { queryParams: { project: id, type: 'chart', from: 'datastore' } });
      }
    }));
  }
  ...
  // 删除单个下载数据
  delete(item) {
    this._http.delete(`${this._api.getDownloadList()}${item.Id}`, this.httpOptions)
      .pipe(
        catchError(err => {
  ...
  deleteProject(id) {
    ...
    this._modalService.onHidden.pipe(take(1)).subscribe(() => {
      // 走的是批量删除的接口
      if (this.bsModalRef.content.confirmFlag) {
        this._http.post(`${this._api.getUploadList()}`, { 'action': 'DELETE', 'data': arr }, this.httpOptions)
          .pipe(
            catchError(err => {
              this.toastr.error(null, '删除失败');
              return err;
