...
export class FavoriteComponent implements OnInit {
  ...
  constructor(private _http: HttpClient, private _modalService: BsModalService, private _api: API) {}
  ...
  search(title, page: number = 1) {
    ...
    this._http
      .get(`${this._api.getFavoriteCaseList(this.dataType)}&page=${page}&title=${title}`, this.httpOptions)
      .subscribe((data) => {
        this.listTemplates = data['data']['res']
        this.searchResult = data['data']['count'] === 0
        if (this.dataType === 'xlsx') {
          this.dataTemplates = data['data']['res']
        } else {
          this.dataReportTemplates = data['data']['res']
        }
      })
  }
  ...
  getFavoriteData(type, page: number = 1) {
    this.titleSearch = ''
    this._http.get(`${this._api.getFavoriteCaseList(type)}&page=${page}`, this.httpOptions).subscribe((data) => {
  ...
  allDelete() {
    ...
    let text = this.dataType === 'xlsx' ? '数据' : '数据指南'
    ...
    this._modalService.onHidden.pipe(take(1)).subscribe((r: string) => {
      if (this.bsModalRef.content.confirmFlag) {
        ...
        this._http.post(`${this._api.getFavoriteList()}`, formData, this.httpOptions).subscribe((data) => {
