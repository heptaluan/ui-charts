...
export class FavoriteTemplateComponent implements OnInit {
  ...
  allDelete() {
    ...
    this._modalService.onHidden.pipe(take(1)).subscribe((r: string) => {
      if (this.bsModalRef.content.confirmFlag) {
        this._http.post(`${this._api.caseLike()}/bulk_del`, { caseid_list: this.listIds }, this.httpOptions)
        .pipe(
          catchError(err => {
  ...
  getFavoriteList(page: number = 1, isFirstPage: boolean = false, searchWord = '') {
    ...
    this._http.get(url, this.httpOptions)
      .subscribe(async data => {
        if (data['resultCode'] === 1000) {
          this.totalNum = data['data']['total'];
          this.myFavoriteList = !isFirstPage ? this.myFavoriteList.concat(data['data']['list']) : _.cloneDeep(data['data']['list']);
        }
