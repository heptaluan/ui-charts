...
import { ContactUsModalComponent, DescriptionComponent, PublishModalComponent, RenameComponent } from '../../../components/modals';
...
export class ProjectManagementComponent implements OnInit {
  ...
  getDataByType(type) {
    ...
    switch (type) {
      case 'all':
        data = this.searchWord ? this.allProjects.filter(item => item.title.indexOf(this.searchWord) > -1) : this.allProjects;
        break;
      case 'info':
        data = this.searchWord ? this.infoProjects.filter(item => item.title.indexOf(this.searchWord) > -1) : this.infoProjects;
        break;
      case 'chart':
        data = this.searchWord ? this.chartProjects.filter(item => item.title.indexOf(this.searchWord) > -1) : this.chartProjects;
        break;
      default:
        break;
    }
  ...
  createInfoTemplateHandle(e, item) {
    ...
    if (this.isIframe) {
      ...
      if (item.type === 'infographic') {
        tempPage.location.href = window.location.href.split('#')[0] +
        '#/pages/workspace?project=' + item.id + '&type=infographic';
      } else {
        tempPage.location.href = window.location.href.split('#')[0] + '#/pages/workspace?project=' + item.id + '&type=chart&from=old';
      }
    }
  }
  ...
  lockProject(item) {
    ...
    this._http.put(url, params, this.httpOptions)
      .subscribe(res => {
        // 处理展示数组
