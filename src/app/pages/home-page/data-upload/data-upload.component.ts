...
export class DataUploadComponent implements OnInit {
  ...
  search(title, page: number = 1) {
    ...
    this._http.get(`${this._api.getUploadList()}?title=${title}&page=${page}`, this.httpOptions).subscribe((data) => {
  ...
  getUploadData(page: number = 1) {
    ...
    this._http.get(`${this._api.getUploadList()}?page=${page}`, this.httpOptions).subscribe((data) => {
      this.listTemplates = data['data']['results']
      this.progressbarValue = data['data']['count']
  ...
  allDelete() {
    ...
    this._modalService.onHidden.pipe(take(1)).subscribe((r: string) => {
      if (this.bsModalRef.content.confirmFlag) {
        this._http
          .post(`${this._api.getUploadList()}`, { action: 'DELETE', data: this.listIds }, this.httpOptions)
          .pipe(
            catchError((err) => {
  ...
  dataUpload() {
    ...
    if (this.progressbarValue >= this.templateTotal) {
      //项目超限提醒
      if (this.isVpl === 'None') {
