...
export class TemplatesListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // 获取对应的场景列表（全部或者分类）
    this.getByConditionsTemplates(this.infoScene_id, this.infoParent_id, this.infoLevel, this.infoOrder, this.infoPrice, this.infoType);
  ...
  handleScroll() {
    ...
    this.timer = setTimeout(() => {
      ...
      if (this.infoPage && top >= maxHeight) {
        this.infoPage++;
        this.getByConditionsTemplates(this.infoScene_id, this.infoParent_id, this.infoLevel, this.infoOrder, this.infoPrice, this.infoType, this.infoPage, this.inputValue);
      }
    }, 50)
  ...
  onSearchEmitValue(tag) {
    ...
    if (tag) {
      this.inputValue = tag;
      this._http.get(`${this.infoConditionUrl}?scene_id=${this.infoScene_id}&parent_id=${this.infoParent_id}&level=${this.infoLevel}&order=${this.infoOrder}&price=${this.infoPrice}&type=${this.infoType}&tag=${tag}&pagenum=${this.infoPage}`)
        .subscribe(data => {
  ...
  getInfoClassify() {
    // 获取信息图分类
    this._http.get(this.infoClassifyUrl, this.httpOptions)
      .subscribe(data => {
  ...
  changeInfoClassify(index, item) {
    ...
    if (item.level === 1) {
      // 百度统计
      window['_hmt'].push(['_trackEvent', 'templatecenter', 'template-list', 'infor-popup-change-template-byscene', item.id]);
      // 改变第一级的时候，第二级第三级第四级初始化
      this.infoClassifyIndex = index;
      this.infoScene_id = item.id;
    } else if (item.level === 2) {
      // 百度统计
      window['_hmt'].push(['_trackEvent', 'templatecenter', 'template-list', 'infor-popup-change-template-byType', (item.id - 6666666000)]);
    ...
    this.getByConditionsTemplates(this.infoScene_id, this.infoParent_id, this.infoLevel, this.infoOrder,
      this.infoPrice, this.infoType, this.infoPage, this.inputValue);
  }
  ...
  changeInfoOrder(i, item) {
    ...
    if (order == '最新') {
      // 百度统计
      window['_hmt'].push(['_trackEvent', 'templatecenter', 'template-list', 'infor-popup-change-order-byIndex', 1]);
      this.infoOrder = 'date';
      this.infoOrderIndex = 0;
    } else if (order == '最热') {
      // 百度统计
      window['_hmt'].push(['_trackEvent', 'templatecenter', 'template-list', 'infor-popup-change-order-byIndex', 2]);
      this.infoOrder = 'hot';
  ...
  getByConditionsTemplates(scene_id, parent_id, level, order, price, type, page: number = 1, tag: string = '') {
    this._http.get(`${this.infoConditionUrl}?scene_id=${scene_id}&parent_id=${parent_id}&level=${level}&order=${order}&price=${price}&type=${type}&tag=${tag}&pagenum=${page}`, this.httpOptions)
      .subscribe(data => {
        this.updateMasonryLayout = false;
        setTimeout(() => {
          this.updateMasonryLayout = true;
  ...
  createInfoTemplateHandle(item) {
    ...
    // 百度统计
    window['_hmt'].push(['_trackEvent', 'templatecenter', 'template-list', 'infor-popup-use-template']);
  ...
  previewInfo(e, item) {
    e.stopPropagation();
    // 百度统计
    window['_hmt'].push(['_trackEvent', 'templatecenter', 'template-list', 'infor-popup-preview-template']);
  ...
  toggleLike(e, item) {
    e.stopPropagation();
    // 百度统计
    window['_hmt'].push(['_trackEvent', 'templatecenter', 'template-list', 'infor-popup-like-template']);

    if (this.userName) {
      this.isLike = !this.isLike;
      this._store.dispatch(new CaseActions.ToggleLikeCaseAction({
        type: 'infographic',
        caseid: item.id,
  ...
  mouseEnterHandler(item) {
    this.getInfoDetailTimer = setTimeout(() => {
      ...
      this.infoDetailSubscription.add(this._store.select(fromRoot.getProjectCaseDetail).pipe(filter(data => !!data)).subscribe(data => {
