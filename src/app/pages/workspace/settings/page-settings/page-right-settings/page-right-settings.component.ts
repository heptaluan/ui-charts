...
export class PageRightSettingsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // 判断传过来的空白模板类型
    const receiveType = this._router.snapshot.queryParams.infoType
      ? this._router.snapshot.queryParams.infoType
      : this.design.sizeType
  ...
  onSizeChanged(event) {
    if (this.project.type === 'infographic') {
      window['_hmt'].push([
        ...
        'editpage',
        'edit-desktop',
        'edit-right-infographic-size-change-index',
        event,
      ])
    } else {
      // 百度统计
      window['_hmt'].push([
        ...
        'editpage',
        'edit-desktop',
        'edit-right-singlechart-size-change-index',
        event,
      ])
    }
    if (event) {
