...
export class ChartLeftSettingsComponent implements TabComponent, OnInit, AfterViewInit, OnDestroy {
  ...
  tbAfterChange(dataObj) {
        ...
        // 执行了删除整列操作，根据 dataSrc 重新排列 map 即可
          ...
          if (newBlock.templateId === '5544734748594536332') {
            for (let i = 0; i < dataSrc.length; i++) {
              if (i === 0) {
                newDataMap.push({ "name": "X轴对象", "index": i, "allowType": ["string"], "isLegend": false, "function": "objCol", "configurable": true })
              } else if (i === 1) {
                newDataMap.push({ "name": "折线列", "index": i, "allowType": ["number"], "isLegend": false, "function": "vCol", "configurable": true })
              } else {
                newDataMap.push({ "name": "数值列", "index": i, "allowType": ["number"], "isLegend": false, "function": "vCol", "configurable": true })
              }
            }
            // 斜率图
            ...
            for (let i = 0; i < dataSrc.length; i++) {
              if (i === 0) {
                newDataMap.push({ "name": "X轴对象", "index": i, "allowType": ["string"], "isLegend": true, "function": "objCol", "configurable": true })
              } else if (i === 1) {
                newDataMap.push({ "name": "数值列", "index": i, "allowType": ["number"], "isLegend": false, "function": "vCol", "configurable": true })
              } else {
                newDataMap.push({ "name": "数值列", "index": i, "allowType": ["number"], "isLegend": false, "function": "vCol", "configurable": true })
          ...
          // 气泡图
            ...
            for (let i = 0; i < dataSrc.length; i++) {
              if (i === 0) {
                newDataMap.push({ "name": "对象", "index": i, "allowType": ["string"], "isLegend": true, "function": "objCol", "configurable": true })
              } else {
                newDataMap.push({ "name": "数值列", "index": i, "allowType": ["number"], "isLegend": false, "function": "vCol", "configurable": true })
              }
            }
      ...
      // 如果大于，执行新增操作，默认在尾部添加一个最新 map
        ...
        if (newBlock.templateId === '5544734748594536332') {
          ...
          for (let i = 0; i < (dataLength - mapLength); i++) {
            dataMap.push({ "name": "柱图列", "index": dataMap.length, "allowType": ["number"], "isLegend": false, "function": "vCol", "configurable": true })
          }
          let index: number = 0;
          for (let i = 0; i < dataMap.length; i++) {
            if (i === 1) {
    ...
    else if (newBlock.templateId === '8000000011111111001' || newBlock.templateId === '8000000011111111002' || newBlock.templateId === '8000000011111111003') {
      let newDataPictures
