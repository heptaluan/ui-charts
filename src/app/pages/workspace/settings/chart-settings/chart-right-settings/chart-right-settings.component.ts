...
export class ChartRightSettingsComponent implements TabComponent, OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.projectType === 'chart') {
      ...
      this.getCurrentProjectSubscription
        .add(this._store.select(fromRoot.getCurrentChartArticle).subscribe(project => {
    ...
    if (temId === '8000000011111111001' || temId === '8000000011111111002' || temId === '8000000011111111003' || temId === '8000000011111111004') {
      // 重新加载动态图表的时候，让动画归位
  ...
  ngAfterViewInit(): void {
    let scrollTop =  $(".chart-setting-content")[0].getBoundingClientRect().height - $(".boleft")[0].getBoundingClientRect().height;
    $(".panel-heading").click(()=>{      
  ...
  parseData() {
    ...
    // 动画面板处理
    if (this.newBlock.templateId === '8000000011111111001' ||
    ...
    if (this.newBlock.props.timerControl && this.newBlock.props.timerControl.options) {
      ...
      this.animationRankmoveTransitionDuraion = [
        ["rankmoveTransitionDuraion", "换位动画", 0.5]
      ]
    }

    // 动画时间轴
    if (this.newBlock.props.map) {
      let mapIndex = this.newBlock.props.map[0][_.findIndex(this.newBlock.props.map[0], (o: any) => {
      ...
      if (isBarLineChartFunc(this.newBlock)) {
        ...
        if (dataMap.length === 1) {
          for (let i = 1; i <= dataMapLength; i++) {
            dataMap.push({ "name": "数值列", "index": i, "allowType": ["number"], "isLegend": false, "function": "vCol", "configurable": true });
          }
        }
  ...
  parseColorList() {
    ...
    if (this.newBlock.props.pictures == undefined) {
      if (this.newBlock.props.colors.type === 'linear') {
        if ((typeof this.colorLists[0] === "string" && this.colorLists[0].indexOf("angle") > -1) 
  ...
  parseLabel() {
    ...
    if (this.newBlock.props.label && this.newBlock.props.label.positionOptions) {
      ...
      // 下拉列表默认位置
      let labelPositionSelectedIndex = this.newBlock.props.label.positionOptions.indexOf(this.newBlock.props.label.positionChoice);
    ...
    if (this.newBlock.props.label && this.newBlock.props.label.barpositionOptions) {
      ...
      // 下折柱混合拉 设置下拉默认位置
      const labelBarPosition = this.newBlock.props.label.barpositionOptions.indexOf(this.newBlock.props.label.barpositionChoice);
    ...
    if (this.newBlock.props.label && this.newBlock.props.label.linepositionOptions) {
      ...
      // 下折柱混合拉 设置下拉默认位置
      const labelLinePosition = this.newBlock.props.label.linepositionOptions.indexOf(this.newBlock.props.label.linepositionChoice);
    ...
    if (isLabelFour(this.newBlock)) {
      ...
      let labelPositionSelectedIndex1 = this.newBlock.props.label.pc_level1Label.positionOptions.indexOf(this.newBlock.props.label.pc_level1Label.positionChoice);
      ...
      let labelPositionSelectedIndex2 = this.newBlock.props.label.pc_level2Label.positionOptions.indexOf(this.newBlock.props.label.pc_level2Label.positionChoice);
      ...
      let labelPositionSelectedIndex4 = this.newBlock.props.label.timeLabel.positionOptions.indexOf(this.newBlock.props.label.timeLabel.positionChoice);
  ...
  onSizeChanged(event) {
    ...
    if (event.type === 'locked') {
      this.newBlock.props.size.ratio = event.locked ? (this.newBlock.props.size.height / this.newBlock.props.size.width) : null;
  ...
  onDropDownChanged(label, index) {
    ...
    if (
      ...
      if (this.mapDropdownLists[index] === '(空)') {
        this.newBlock.props.map[0][_.findIndex(this.newBlock.props.map[0], function (o: any) {
          return o.name === label
        })].index = null
        ...
        if (this.curBlock.templateId === '7955555555502346002' || this.curBlock.templateId === '4612096174443311107' || this.curBlock.templateId === '7955555777700001201') {
          this.showChartLegend = false
  ...
  getChangeColorLegendList() {
    ...
    switch (this.newBlock.templateSwitch) {
      ...
      case 'obj-n-value-time':
        // 动态圆堆积图需要单独处理
        let mapIndex = 1
        let levelOen = this.newBlock.props.map[0][_.findIndex(this.newBlock.props.map[0], (o: any) => {
          return o.name === '层级1'
        })].index
        let levelTwo = this.newBlock.props.map[0][_.findIndex(this.newBlock.props.map[0], (o: any) => {
          return o.name === '层级2'
        })].index
        let levelThree = this.newBlock.props.map[0][_.findIndex(this.newBlock.props.map[0], (o: any) => {
          return o.name === '层级3'
        })].index
  ...
  handleChartPreviewButton() {
    ...
    if (!this.animationPreviewButtonDisabled) {
      ...
      const time = (~~this.newBlock.props.animation.duration + ~~this.newBlock.props.animation.startDelay + ~~this.newBlock.props.animation.endPause) * 1000
  ...
  handleChangeDynamicChartRankmoveTransitionDuraion(e) {
    this.animationRankmoveTransitionDuraion = [
      ["rankmoveTransitionDuraion", "换位动画", Number(e)]
    ]
    this.newBlock.props.timerControl.options = [
      ["rankmoveTransitionDuraion", "换位动画", Number(e)]
    ]
    this.updateCurBlock('redo');
  }
  ...
  fixOldChartJSON() {
    ...
    if (this.newBlock.props.label.display !== undefined && this.newBlock.props.label.textLabel !== undefined && this.newBlock.props.label.numberLabel !== undefined) {
      ...
      } else { // textLabel 没有 show 删除 display textLabel
  ...
  getStepOfLabelNum(axisType?) {
    ...
    let numList = [];
    let start, stop, count=5;
    if (this._curBlock.templateId === "5544734748594536495" || this._curBlock.templateId === "154778102528502157" || this._curBlock.templateId === "154778025133261039") {
