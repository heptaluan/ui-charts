  ...
  ngOnInit() {
    this.mySubscription = this.chartTemplates$.subscribe((data) => {
      if (data) {
        ...
        if (
          templateSwitch === 'tree' ||
          templateSwitch === 'link-obj-valued' ||
          templateSwitch === 'link-typed' ||
          ...
          this.chartTemplates = data.filter(
            (item) => item.templateSwitch === 'cross' || item.templateSwitch === 'cross-time'
          )
        } else if (templateSwitch === 'obj-n-value-time') {
          this.chartTemplates = data.filter((item) => item.templateSwitch === 'obj-n-value-time')
        } else if (templateSwitch === 'cross') {
          // 除了 obj-n-value-time tree link-typed link-obj-valued 特殊处理不推荐
          ...
          this.chartTemplates = data
            .filter((item) => item.templateSwitch !== 'sunburst')
            .filter((item) => item.templateSwitch !== 'tree')
