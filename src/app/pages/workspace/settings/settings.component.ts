 ...
 */
import { Component, OnInit, OnDestroy, AfterViewInit, ViewChild, ComponentFactoryResolver, Input, ComponentRef } from '@angular/core';
...
export class SettingsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.projectType === 'infographic') {
      this.projectSubscribe = this._store.select(fromRoot.getCurrentProjectFull).pipe(
      ...
      this.projectSubscribe = this._store.select(fromRoot.getCurrentChartProjectFull).pipe(
        filter((project: ProjectModels.ProjectInfo) => project && project.id === this.projectId))
        .subscribe(project => {
          project.public_url = project.public_url.replace('dydata.io', 'dycharts.com')
          project.private_url = project.private_url.replace('dydata.io', 'dycharts.com')
  ...
  onSelected(item: ProjectModels.ProjectContentObject, projectUpdate: boolean = false) {
    ...
    if (this.lastItem && this.lastItem.type === item.type
  ...
  selectCurBlock(blockId: string) {
    ...
    if (this.projectType === 'infographic') {
      this.projectSubscribe = this._store.select(fromRoot.getCurrentProjectFull).pipe(
        filter((project: ProjectModels.ProjectInfo) => project && project.id === this.projectId)).pipe(take(1))
        .subscribe(project => {
          newProject = project
        });
    } else if (this.projectType === 'chart') {
