     ...
     // 启用透明度单独配置 加入颜色模块统一渲染
      if (this.opacity !== 100){
        if (this.colors[0].length === 4) {
          this.colors[0] = this.colors[0][0] + this.colors[0][1] + this.colors[0][1] 
                                             + this.colors[0][2] + this.colors[0][2] 
                                             + this.colors[0][3] + this.colors[0][3] 
        }
        this.curBlock.props.fill.fillColor[0] = this.colors[0].substring(0,7) + Math.round(this.opacity*2.55).toString(16).padStart(2,"0") ;
      ...
      this.borderColors = this.curBlock.props.strokeColor === 'transparent' ? '#c759a1ff' :this.curBlock.props.strokeColor;
      ...
      // 显示对应配置项
      if (this.curBlock.shapeType === 'oval' || this.curBlock.shapeType === 'triangle' || this.curBlock.shapeType === 'pentagon' || this.curBlock.shapeType === 'icon') {
  ...
  onSizeChanged(event) {
    ...
    switch (this.curBlock.shapeType) {
      ...
      case 'triangle':
        ...
        this.curBlock.path = `${triangleBoxWidth / 2} 0, 0, ${triangleBoxHeight}, ${triangleBoxWidth}, ${triangleBoxHeight}`;
        break;
      ...
      case 'pentagon':
        ...
        this.curBlock.path = `${pentagonBoxWidth / 2} 0, ${pentagonBoxWidth} ${(pentagonBoxHeight) * 0.4}, ${(pentagonBoxWidth) * 0.85} ${pentagonBoxHeight}, ${(pentagonBoxWidth) * 0.15} ${pentagonBoxHeight}, 0 ${(pentagonBoxHeight) * 0.4}`;
        break;
    ...
    if (this.curBlock.shapeType === 'line') {
      ...
      switch (event) {
        case 0:
          ...
          if (
            this.curBlock.shapeType === 'rectangle' ||
            this.curBlock.shapeType === 'round-rectangle'
          ) {
