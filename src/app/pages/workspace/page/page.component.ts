...
export class PageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.mysubscription2
          ...
          if (themeList) {
            ...
            if (!this.project.article.contents.theme) {
              ...
              this._store.dispatch(
                new ProjectActions.UpdateAndExitCurrentProjectAction(newProject.id, {
                  action: 'save_project',
      ...
      .add(
        this._dataTransmissionService.getGroupData().subscribe((isGroup) => {
          if (isGroup) {
            // 打散
            this.disperseGroup()
          } else {
  ...
  setPageStyle() {
    ...
    this._renderer.setStyle(
      ...
      'height',
      this.setting.height * (scale / 100) + 'px'
    )
    this._renderer.setStyle(
      this._el.nativeElement.getElementsByClassName('page')[0],
      'width',
      this.setting.width * (scale / 100) + 'px'
    )
    this._renderer.setStyle(
      this._el.nativeElement.getElementsByClassName('page')[0],
      'background',
      ...
      if (isOnMouseDown) {
        if (pos === 'bottom') {
          ...
          const newWidth = Math.floor(
            Number(this.originWidth) + ((Number(newClientX) - Number(oldClientX)) / scale) * 2
          )
          if (newWidth <= 300) {
  ...
  copy() {
    ...
    const newBlock = _.cloneDeep(
      this.blockLists.find((x) => x.blockId === (this.selectedDom as HTMLElement).getAttribute('chartid'))
    )
    ...
    this._modalService.onHidden.pipe(take(1)).subscribe((r: string) => {
      if (this.bsModalRef.content.confirmFlag) {
        ...
        if (title) {
          ...
          this._http.post(url, forkChart, { withCredentials: true }).subscribe((res) => {
            console.log(res)
  ...
  groupDelete() {
    ...
    this._store.dispatch(
      new ProjectActions.UpdateCurrentProjectGroupDeleteAction({
        groupList: groupList,
  ...
  groupCopy() {
    ...
    this.selectedDomList.map((item) => {
      const newBlock = _.cloneDeep(
        that.blockLists.find((x) => x.blockId === (item as HTMLElement).getAttribute('chartid'))
      )
      newBlock.blockId = this._utilsService.generate_uuid()
  ...
  disperseGroup() {
    ...
    children.forEach((block, index) => {
      ...
      if (diffRotate === 0) {
        ...
        const blockCenterPoint = this.getRectCenterPoint(
          Number(blockWidth),
          Number(blockHeight),
          Number(blockLeft) + diffLeft,
          Number(blockTop) + diffTop
        )
        // 计算block 中心点 距离外层中心点偏差的角度
        const degree =
          (Math.atan((blockCenterPoint.y - outerCenterPoint.y) / (blockCenterPoint.x - outerCenterPoint.x)) / Math.PI) *
          180
  ...
  generateGroup() {
    ...
    const {
      ...
      width: actualWidth,
      height: actualHeight,
    } = document.querySelector('.drag-box-wrap').getBoundingClientRect()
  ...
  // 获取指定 blockId 的长宽信息
  getBlockData(id) {
    return Array.from(document.querySelectorAll('.group-container-item'))
