...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.mySubscription
      ...
      .add(
        this._dataTransmissionService.getSaveLoading().subscribe((res) => {
          if (res === 0) {
            this.loadState = 0
            this.savingTip = '正在保存'
            this.timeTip = ''
  ...
  saveProject(type?) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (type === 'goDownload') {
          that._router.navigate(['pages', 'download'], {
            queryParams: { project: that.projectId, type: that.projectType },
          })
