...
export class WorkspaceComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    if (this.projectType === 'infographic') {
      ...
      this.mySubscription.add(
        this._store
          ...
          .filter((project) => !!project && project.id === this.projectId)
          .subscribe((project: ProjectModels.ProjectInfo) => {
  ...
  showChartLeftSideBar() {
    this.modalService.show(ContactUsModalComponent, {
      initialState: {
        content:
          '图表项目仅开放「图表」的选择和编辑功能。若需要添加「文字」「形状」「图片」等元素，您可以尝试创建<a style="color: #007bff;text-decoration: underline;" target="_blank" href="https://dycharts.com/appv2/#/pages/home/info-template">数据图文</a>项目。',
        title: {
          position: 'center',
          content: '提示',
          button: '确定',
  ...
  goHome() {
    ...
    if (this.project.article.contents.pages[0].blocks.length !== $('lx-block').length) {
      console.error(
        '项目管理：当前保存的block数量与页面显示的不相等',
        `保存的数量：${this.project.article.contents.pages[0].blocks.length}`,
        `展示的数量：${$('lx-block').length}`
      )
    }
    if (this.projectType === 'infographic') {
