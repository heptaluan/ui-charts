...
export class referLineDirective implements OnInit {
  ...
  ngOnInit() {
    ...
    this.referSubscription.add(
      this._dataTransmissionService.getPageScaleChange()
      .subscribe(() => {
        // 变化后的比例
  ...
  onMousemove(event) {
    if (this.isDown) {
      ...
      if (this.referLine === 'row') {
        // 处理移动
        const leftx = (document.querySelector('#focal').getBoundingClientRect() as any).x - (document.querySelector('.left-content').getBoundingClientRect() as any).width - (document.querySelector('.scrollable').getBoundingClientRect() as any).width;
