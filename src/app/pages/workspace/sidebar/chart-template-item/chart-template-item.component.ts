...
export class ChartTemplateItemComponent implements OnInit, OnChanges {
  ...
  filterChartTemplates(arr?) {
    ...
    _.forEach(filterArr, (item) => {
      if (item.chart_type === '动态图表') {
        ...
        lineList.push(item)
      } else if (item.chart_type === '柱状图') {
        barList.push(item)
      } else if (item.chart_type === '面积图') {
        areaList.push(item)
      } else if (item.chart_type === '散点图') {
        splashesList.push(item)
      } else if (item.chart_type === '极坐标图') {
        polarList.push(item)
      } else if (item.chart_type === '关系图') {
        relationList.push(item)
      } else if (item.chart_type === '仪表盘') {
        panelList.push(item)
      } else if (item.chart_type === '树图') {
        treeList.push(item)
      } else if (item.chart_type === '桑基图') {
        sankeyList.push(item)
      } else if (item.chart_type === '漏斗图') {
        funnelList.push(item)
      } else if (item.chart_type === '热力图') {
        hotList.push(item)
      } else {
        otherList.push(item)
      }
    ...
    // 功能性筛选
    _.forEach(filterArr, (item) => {
      if (_.includes(item.function_type, '比较类')) {
        compareList.push(item)
      }
      if (_.includes(item.function_type, '层级类')) {
        shipList.push(item)
      }
      if (_.includes(item.function_type, '分布类')) {
        branchList.push(item)
      }
      // if (_.includes(item.function_type, '比例类')) { ratioList.push(item) }
      if (_.includes(item.function_type, '趋势类')) {
        timeList.push(item)
      }
      if (_.includes(item.function_type, '占比类')) {
        proportionList.push(item)
      }
      if (_.includes(item.function_type, '流向类')) {
        flowList.push(item)
      }
    })
  ...
  insertInfoChartHandle(blockId, templateId) {
    ...
    this.insertInfoChartSubscription.add(
  ...
  insertSingleChartHandle(blockId, templateId) {
    ...
    this.insertSingleChartSubscription.add(
  ...
  toggleChartHandle(blockId, templateId) {
    ...
    // 获取项目信息
    if (this.projectType === 'infographic') {
      this.toggleChartSubscription.add(
        this._store.select(fromRoot.getCurrentProjectFull).subscribe((project) => {
          this.project = project
        })
      )
    ...
    if (newBlock.templateSwitch !== 'cross' || newBlock.props.colors.type !== 'single') {
      if (
        this.curBlock.props.colors &&
        newBlock.props.colors &&
        newBlock.props.colors.list &&
        this.curBlock.props.colors.list
      ) {
        let colorList = []
