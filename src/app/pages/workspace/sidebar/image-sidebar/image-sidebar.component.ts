...
export class ImageSidebarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // 监听手机是否已经扫码
    this.getCodeSubjectStateSubscription.add(this._dataTransmissionService.getCodeSubjectState().subscribe(res => {
      if (res.code === 0) {
  ...
  search(title, nextSearchPage: string = '') {
    ...
    if (!nextSearchPage) {
      if (title) {
        this._http.get(`${this.sysImageUrl}?tag=${title}`, this.httpOptions)
          .subscribe(data => {
            this.imageTemplates = [];
      ...
      this._http.get(this.nextSearchPage, this.httpOptions)
        .subscribe(data => {
