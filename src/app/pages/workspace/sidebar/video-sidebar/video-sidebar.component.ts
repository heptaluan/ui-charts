...
export class VideoSidebarComponent implements OnInit {
  ...
  url =
    '<iframe src="//player.bilibili.com/player.html?aid=371053613&bvid=BV1xZ4y1H7d5&cid=204852140&page=1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"> </iframe>'
  ...
  insertVideo() {
    ...
    if (this.videoText.indexOf('<iframe') > -1) {
      ...
      newBlock.props.size.width =
        this.getIframeWidth(this.videoText) !== 'false'
          ? this.getIframeWidth(this.videoText)
          : newBlock.props.size.width
      newBlock.props.size.height =
        this.getIframeHeight(this.videoText) !== 'false'
          ? this.getIframeHeight(this.videoText)
          : newBlock.props.size.height
      // 设置 ratio
