...
export class SceneSidebarComponent implements OnInit {
  ...
  initGetTemplateList(pageNum = 1, flag = true, scene_id = 0, tag = '') {
    ...
    this._http
      .get(`${this.infoConditionUrl}?scene_id=${scene_id}&pagenum=${pageNum}&tag=${tag}`, this.httpOptions)
      .subscribe((data) => {
  ...
  initGetFavTemplateList(pageNum = 1, flag = true, title = '') {
    ...
    this._http.get(`${this.favTemplateUrl}&pagenum=${pageNum}&title=${title}`, this.httpOptions).subscribe((data) => {
      // 用一个数组来存放未处理的数据
          ...
          if (arr.length === dataLength) {
            for (let i = 0; i < dataLength; i++) {
              ...
              if (i < 2 && that.favPageNum === 1) {
                ...
                // top 赋值完成后，将新的高度替换原来最短的
                that.hFavArr[that.hFavArr.indexOf(minH)] =
                  that.copyFavTemplates[i].top + that.copyFavTemplates[i].height
              }
              // i 等于数据最大长度，将处理好的数据赋值给原数组或者新增
              if (i === dataLength - 1) {
                that.favPageNum++
