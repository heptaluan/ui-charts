...
export class ReferComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // 监听添加参考线
    this._dataTransmissionService.getReferLine().subscribe((res) => {
      if (res === 'deleteAll') {
  ...
  addLine(direction = 'row') {
    ...
    const {
      ...
      height: H,
      width: W,
    } = (document.querySelector('#focal') as HTMLElement).getBoundingClientRect()
    const CH = document.documentElement.clientHeight
    const CW =
      document.documentElement.clientWidth -
      (document.querySelector('.right-content') as HTMLElement).getBoundingClientRect().width
  ...
  update() {
    this._store.dispatch(
      new ProjectActions.SetProjectReferLineAction({
