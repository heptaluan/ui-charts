...
export class InnerHeaderComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.savingStateSubscription.add(this._store.select(fromRoot.getProjectSavingState).subscribe(saving => {
      setTimeout(() => {
    ...
    if (this.selectItems) {
      this.selectSubscription = this.selectItems.subscribe(objects => {
        ...
        if (!objects[0]['newData'] && objects[0]['type'] !== "multiple") {
          ...
          if ($('.right-content').css('width') === '280px' && (oldBlockId !== objects[0]['blockId'] || objects[0]['type'] === "article")) {
    ...
    if (changes.topbarEmitData) {
      if (changes.topbarEmitData.currentValue) {
        ...
        if (changes.topbarEmitData.previousValue && changes.topbarEmitData.previousValue.type) {
          if ((changes.topbarEmitData.currentValue.type === 'audio' && changes.topbarEmitData.previousValue.type === 'chart') || (changes.topbarEmitData.currentValue.type === 'chart' && changes.topbarEmitData.previousValue.type === 'audio')) {
            $('.right-content').css('width', '240px')
          }
        }
      }
  ...
  executeUndoAction() {
    ...
    this._store.select(fromRoot.getRedoUndoUpdateTypeAndData).pipe(take(1)).subscribe(res => {
      if (res.target && res.target.type === 'group-delete') {
        ...
        this._blockService.backNode(res.target.indexData.moveArr, res.target.indexData.oldList, this.blockLists, res.target.indexData.changeAction);
        // 更新数据
        ...
        this._store.dispatch(new ProjectActions.SaveCurrentProjectAritcleAction({
          article: newArticle
        }));
  ...
  executeRedoAction() {
    ...
    this._store.select(fromRoot.getRedoUndoUpdateTypeAndData).pipe(take(1)).subscribe(res => {
      if (res.target && res.target.type === 'group-delete') {
        ...
        this._blockService.moveNode(_.cloneDeep(res.target.indexData.moveArr), res.target.indexData.changeAction, true);
        // 更新数据
        let oldContents = _.cloneDeep(res.data.contents ? res.data.contents : res.data.article.contents);
    ...
    $(document).keydown(function (event) {
      ...
      if ($('.edit-data-tab .nav-item:first-child').hasClass('active')) {
        ...
        switch (keyNum) {
          ...
          case 8:
            // 判断具有focuse对象
            if ($('.block-container').hasClass('show') && $(".color-picker-wrapper-show").length === 0 && document.activeElement.localName !== 'input' && document.activeElement["type"] !== 'text' && document.activeElement.localName !== 'textarea') {
              event.preventDefault();
              if (!(that.currentBlock && that.currentBlock.locked)) {
                that.delete();
              }
