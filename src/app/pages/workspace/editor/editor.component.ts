...
import {
  ...
  ViewChild, OnDestroy, ComponentRef, Output, EventEmitter, ComponentFactory, ElementRef, SimpleChanges
} from '@angular/core';
...
// import { BlockComponent } from '../../../share/block/block.component';

// import { ChartMapService } from '../../../charts/chart-map.service';
...
export class EditorComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.projectType === 'infographic') {
      this.mySubscription = this._store.select(fromRoot.getCurrentProjectFull).pipe(
        filter((project: ProjectModels.ProjectInfo) => project && project.id === this.projectId), take(1)).subscribe(project => {
      ...
      this.mySubscription = this._store.select(fromRoot.getCurrentChartProjectFull).pipe(
        filter((project: ProjectModels.ProjectInfo) => project && project.id === this.projectId)).subscribe(project => {
          this.project = project;
          // 避免重复加载组件
          if (this.pageComponentRefList.length === 0) {
            _.each(project.article.contents.pages, (page) => {
  ...
  onEditorSelected(e) {
    if (this.projectType === 'infographic') {
      ...
      if (e.target.classList.contains('workspace') || e.target.classList.contains('edit-area') || e.target.classList.contains('page-container')) {
