...
export class ChartPageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.mySubscription
      ...
      .add(this._store.select(fromRoot.getChartTemplates).subscribe(list => {
        if (list.length > 0) {
      ...
      .add(this._store.select(fromRoot.getCurrentProjectTheme).subscribe(themeList => {
        if (themeList) {
          ...
          if (!this.project.article.contents.theme) {
            ...
            this._store.dispatch(new ProjectActions.UpdateAndExitCurrentChartProjectAction(newProject.id, {
              action: 'save_project',
    ...
    this.mySubscription2.add(stream2.pipe(skip(1), withLatestFrom(stream1, (modifyBlock, newArticle) => {
      ...
      switch (modifyBlock.type) {
        case 'article':
          ...
          break;
        case 'chart':
          diffBlock = _.find((<ProjectModels.InfographicProjectContents>newArticle.contents).pages[0].blocks, { blockId: modifyBlock.blockId });
          setTimeout(() => {
            this.setPageStyle();
          }, 0);
          if (!diffBlock) {
  ...
  setPageStyle() {
    let w = document.querySelector('.page svg') ? document.querySelector('.page svg').getAttribute('width') : this.setting.width
    let h = document.querySelector('.page svg') ? document.querySelector('.page svg').getAttribute('height') : this.setting.height
    ...
    if (this.project.article.contents.pages[0].blocks.length > 0) {
      ...
      if (this.blockProps) {
        ...
        this.logoDisplay = {
          ...
          bottom: -(~~this.blockProps.logoDisplay.imgHeight + ~~this.blockProps.logoDisplay.topLineHeight + ~~this.blockProps.logoDisplay.bottomLineHeight - ~~pcMobileDiff) + 'px'
        }
        ...
        if (this.blockProps.watermarkDisplay.show && this.isShowWatermark) {
          ...
          this._renderer.setStyle(this._el.nativeElement.getElementsByClassName('page')[0], 'background', `${this.setting.backgroundColor} url(${this.blockProps.watermarkDisplay.imgUrl})`)
        } else {
