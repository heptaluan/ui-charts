...
export class CaseComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    this.mySubscription.add(this._store.select(fromRoot.getProjectCaseDetail).pipe(filter(data => !!data)).subscribe(data => {
      this.case = data;
      const previewHeight = 700 / Number(this.case.article.contents.design.width) * this.case.article.contents.design.height + 'px';
      this._render.setStyle(this._el.nativeElement.getElementsByClassName('preview-container')[0], 'height', previewHeight);
  ...
  toggleLike(e, item) {
    ...
    if (this.userName) {
      ...
      window.location.href = `${this._api.getOldUrl()}/vis/auth/signin.html?redirect=`+ encodeURIComponent(`${window.location.href.split('#')[0]}#/pages/case=${item.id}&type=infographic`);
    }
  }
