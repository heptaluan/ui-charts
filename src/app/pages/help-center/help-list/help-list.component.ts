...
export class HelpListComponent implements OnInit {
  ...
  constructor(private _el: ElementRef, private modalService: BsModalService) {}
  ...
  ngOnInit() {
    ...
    if (updateTip) {
      ...
      if (clickTime) {
        ...
        let updateTipTime = new Date(
          updateTip.slice(0, 4) + '/' + updateTip.slice(4, 6) + '/' + updateTip.slice(6, 8)
        ).getTime()
