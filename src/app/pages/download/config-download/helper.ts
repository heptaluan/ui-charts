...
export function handleBaiduCollect(isPhone: boolean, chooseType: exportType, projectType: 'chart' | 'infographic', isHasCompleteId: boolean, isDIY: boolean) {
  if (projectType !== 'chart') { // 信息图
    // 不需要判断pc/手机埋点的情况
    ...
    // 信息图 导出到 PC / phone
    window['_hmt'].push(['_trackEvent', 'editpage', 'edit-download', `edit-download-${isPhone ? `${chooseType}-phone` : chooseType}`]);
  } else {
    // 单图 导出 包括 pc/ phone 全格式导出
    window['_hmt'].push(['_trackEvent', 'editpage', 'edit-download', `edit-download-chart-${isPhone ? `${chooseType}-phone` : chooseType}`])
  }
}