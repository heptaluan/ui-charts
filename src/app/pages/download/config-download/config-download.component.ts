...
import { UpgradeMemberComponent, ExportToPhoneModalComponent, ProgressComponent, BusinessPriceComponent } from '../../../components/modals';
...
export class ConfigDownloadComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    ...
    // 获取重试
    this._dataTransmissionService.getRetryStatus()
      .subscribe(res => {
        if (res === 'retry') {
          this.isRetry = true;
  ...
  initDynamicChartData(project) {
    ...
    if (temId === '8000000011111111001' || temId === '8000000011111111002' || temId === '8000000011111111003' || temId === '8000000011111111004') {
      ...
      // 动画时间轴
      let mapIndex = block.props.map[0][_.findIndex(block.props.map[0], (o: any) => {
  ...
  resetTransparent(type) {
    ...
    } else if (this.bgTransparent && type !== 'mov') {    // 只要透明底是勾选，并且不是 mov, 关闭透明底
      this.transparentSwitch();
    }
  }
  ...
  exportImage() {
    // 验证可下载权限后 导出时埋点 
    handleBaiduCollect(this.isExportToPhone, this.chooseType, this.projectType, this.project && this.project.templete_id, this.project.templete_id === '0');
  ...
  toImage(type, isTransparent, width, height) {
    ...
    if (type === 'jpg') {
      domtoimage.toJpeg($('.page-box')[1], {  
        width: width,
        height: height,
        quality: 0.5,
        style: {
          'transform-origin': '0 0',
          'transform': `scale(${Number(that.exportImgW / this.imgInitW) / scale}, ${Number(that.exportImgH / this.imgInitH)  / scale})`,
  ...
  uploadImg(blob) {
    ...
    this._http.post(url, fd, { withCredentials: true})
      .subscribe(res => {
        if (res['resultCode'] === 1000) {
  ...
  downloadMp4() {
    ...
    if (this.showDynamicChatAnimation) {
      ...
      this._http.post(this._api.getMp4ByUrl(), data, {withCredentials: true})
        .subscribe(() => {
          setTimeout(() => {
      ...
      if (usedTime >= totalTime * rate) {
        ...
        const timer = setInterval(() => {
          ...
          downloadZipImage(imageList, (blob) => {
            ...
            this._http.post(this._api.postImages(), fd, { withCredentials: true })
              .subscribe(() => {
  ...
  downloadMp4ById(id, flag = false) {
    ...
    this._http.get(this._api.getVideo(id), {withCredentials: true})
      .pipe(
        catchError(e => of(e))
      )
      .subscribe(res => {
        if (res.status === 404) {
          ...
          clearTimeout(timer);
          // 百度统计
          window['_hmt'].push(['_trackEvent', 'editpage', 'edit-download', `edit-download-chart-${this.chooseType}-success`]);
          this._dataTransmissionService.saveProcess('success');
  ...
  handleDynamicTime(data) {
    ...
    if (data.templateId === '8000000011111111004') {
      mapIndex = data.props.map[0][_.findIndex(data.props.map[0], (o) => {
        return o['function'] === 'timeCol'
      })].index;
    ...
    const time = (length-1) * Number(data.props.timerControl.frameDuration) + Number(data.props.animation.startDelay) + Number(data.props.animation.endPause);
    return time;  
  }
