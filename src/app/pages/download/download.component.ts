...
export class DownloadComponent implements OnInit {
  ...
  initData() {
    if (this.projectType === 'infographic') {
      this.contents$ = this._store.select(fromRoot.getCurrentProjectFull).pipe(
      ...
      this.contents$ = this._store.select(fromRoot.getCurrentChartProjectFull).pipe(
        filter((project) => !!project),
        map((project) => {
          ...
          if (project && newProject.article.contents.pages[0].blocks[0]) {
            ...
            if (blockProps.logoDisplay && blockProps.publishDisplay) {
              ...
              const diffHeightNum =
                ...
                ~~pcMobileDiff
              newProject.article.contents.design.height = flag
                ? ~~blockProps.size.height + diffHeightNum + ''
                : blockProps.size.height
