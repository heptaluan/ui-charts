...
export class HttpService {
  ...
  get<T>(url: string, httpOptions?: Object): Observable<any> {
    return this._http.get<CommonResponse<T>>(url, httpOptions)
      .pipe(
  ...
  post<T>(url: string, data: any, httpOptions?: Object): Observable<any> {
    return this._http.post<CommonResponse<T>>(url, data, httpOptions)
      .pipe(
        map((rep => {
          if (rep.resultCode === 1000) {
            return rep;
  ...
  delete<T>(url: string, data: any, httpOptions?: Object): Observable<any> {
    return this._http.delete<CommonResponse<T>>(url, httpOptions)
      .pipe(
        map((rep => {
          if (rep.resultCode === 1000) {
            return rep;
  ...
  put<T>(url: string, data: any, httpOptions?: Object): Observable<any> {
    return this._http.put<CommonResponse<T>>(url, data, httpOptions)
      .pipe(
        map((rep: CommonResponse<T>) => {
          if (rep.resultCode === 1000) {
            return rep;
