...
export class UserEffect {
  constructor(private _service: UserService, private _actions$: Actions, private toastr: ToastrService) {}

  @Effect()
  getUserInfo$: Observable<Action> = this._actions$.pipe(
    ofType(UserActions.GET_USER_INFO),
    switchMap(() =>
      this._service.getUserInfo().pipe(
  ...
  setUserInfo$: Observable<Action> = this._actions$.pipe(
    ...
    switchMap((data: UserActions.SetUserInfoAction) =>
      this._service.setUserInfo(data.payload).pipe(
        map((res) => {
  ...
  getUserBill$: Observable<Action> = this._actions$.pipe(
    ...
    switchMap(() =>
      this._service.getUserBill().pipe(
