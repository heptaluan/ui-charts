  ...
  switch (action.type) {
    ...
    case ProjectActions.UPDATE_CURRENT_PROJECT_ARTICLE_SUCCESS: {
            ...
            if (action.payload.method === 'add') {
              ...
              if (!action.payload.target.hasOwnProperty('target')) {
                setTimeout(() => {
                  ...
                  const newIndex = Array.from(blocks).findIndex(
                    (item) => item['children'][0]['attributes']['chartid'].nodeValue === blockData.blockId
                  )
      ...
      if (updateTarget.target !== 'redo') {
        ...
        if (!newState.allArticleRecords || newState.allArticleRecords.length === 0)
          newState.allArticleRecords = [newCurrentArticle]
    ...
    case ProjectActions.UPDATE_CHART_CURRENT_PROJECT_ARTICLE_SUCCESS: {
      ...
      if (newState.currentChartProject.type === 'chart') {
        ...
        switch (updateTarget.type) {
          case 'article':
            ;(<ProjectModels.InfographicProjectContents>newState.currentChartArticle.contents).design =
              updateData.design
            break
          default:
            const blockId = updateTarget.blockId
