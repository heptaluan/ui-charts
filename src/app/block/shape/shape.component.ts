  ...
  initShapeData() {
    ...
    if (this.data.setting) {
      ...
      this.shapeBoxStyles = {
        ...
        'filter': dataProps.shadow && dataProps.shadow.display ? `drop-shadow(
                                                ...
                                                ${Math.round(dataProps.shadow.shadowRadius * Math.sin(dataProps.shadow.shadowAngle * Math.PI / 180))}px 
      ...
      let svgPath = this._el.nativeElement.querySelector('.svgIcon svg path');
      if (svgPath) {
        svgPath.style.fill = isGradient ? `url(#${this.data.setting.blockId})` : this.colorToRGBA(dataProps.fill.fillColor[0]);
      }
    }
  }
    ...
    objs.each(function (index, dragEleDom) {
      ...
      $(document).mouseup(function (e) {
        ...
        if (_move && _moving) {
          ...
          // 按钮
          const lineOneElClientRect = that._el.nativeElement.parentElement.parentElement.querySelector('.line-item1').getBoundingClientRect();
          const lineOneLeft = lineOneElClientRect.left - pageLeft
          const lineOneTop = lineOneElClientRect.top - pageTop

          const lineTwoElClientRect = that._el.nativeElement.parentElement.parentElement.querySelector('.line-item2').getBoundingClientRect();
          ...
          block.dragPosition.oneLeft = parseInt($(that._el.nativeElement.parentElement.parentElement).find('.line-item1').css('left'))
          block.dragPosition.oneTop = parseInt($(that._el.nativeElement.parentElement.parentElement).find('.line-item1').css('top'))
          block.dragPosition.twoLeft = parseInt($(that._el.nativeElement.parentElement.parentElement).find('.line-item2').css('left'))
          block.dragPosition.twoTop = parseInt($(that._el.nativeElement.parentElement.parentElement).find('.line-item2').css('top'))
