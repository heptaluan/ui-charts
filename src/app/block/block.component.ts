...
export class BlockComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._notifyChartRenderService.getChartRender().subscribe((res) => {
      if (
        (this.componentRef.instance.echart && this.projectType === 'infographic' && this.isSelected) ||
        this.projectType === 'chart'
      ) {
        if (
          this.data.setting.templateSwitch === 'cross-time' ||
          this.data.setting.templateSwitch === 'obj-n-value-time'
        ) {
    ...
    if (this.projectType === 'infographic') {
      this._dataTransmissionService.getIsShowBorder().subscribe((res) => {
        this.isUseHover = res
      })
    }
    ...
    // 只有动态图表才显示动画按钮
    if (
      this.data.setting.templateId === '8000000011111111001' ||
  ...
  ngAfterViewInit() {
    ...
    // 初始化插入时 根据缩放比例进行调整
    if (
      this.data.setting.type === 'chart' ||
      this.data.setting.type === 'text' ||
      this.data.setting.type === 'shape' ||
      this.data.setting.type === 'audio' ||
      this.data.setting.type === 'video'
    ) {
  ...
  buildBlockSettings() {
    ...
    if (this.data.setting) {
      ...
      this.blockStyles = {
        ...
        transform: this.data.setting.props.size.rotate
          ? `rotate(${Number(this.data.setting.props.size.rotate).toFixed()}deg)`
          : `rotate(0deg)`,
      }
