import {
  ...
  ViewEncapsulation, ElementRef, ChangeDetectorRef
} from '@angular/core';
...
export class ColorPickerComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public selectedDefaultColor: string;
  ...
  ngOnInit(): void {
    ...
    const hueHeight = this.hueSlider.nativeElement.offsetHeight;
    const alphaWidth = this.alphaSlider.nativeElement.offsetWidth;

    // this.sliderDimMax = new SliderDimension(hueWidth, this.cpWidth, 130, alphaWidth);
    this.sliderDimMax = new SliderDimension(hueHeight, 130, 96, alphaWidth);
  ...
  ngAfterViewInit(): void {
    if (this.cpWidth !== 130) {
      const lightnessWidth = this.lightnessSlider.nativeElement.offsetWidth;
      const lightnessHeight = this.lightnessSlider.nativeElement.offsetHeight;
      const hueHeight = this.hueSlider.nativeElement.offsetHeight;
      const alphaWidth = this.alphaSlider.nativeElement.offsetWidth;

      // this.sliderDimMax = new SliderDimension(hueWidth, this.cpWidth, 130, alphaWidth);
      this.sliderDimMax = new SliderDimension(hueHeight, lightnessWidth, lightnessHeight, alphaWidth);

      this.updateColorPicker(false);

      this.cdRef.detectChanges();
    }
    const presetColorList = document.querySelectorAll('.preset-color');
    const currentDirectiveElementRefBackgroundColor = this.directiveElementRef.nativeElement.style.backgroundColor;
    for (let i = 0; i < presetColorList.length; i++) {
      if ((presetColorList[i] as HTMLElement).style.backgroundColor === currentDirectiveElementRefBackgroundColor) {
        (presetColorList[i] as HTMLElement).style.border = '#0090ff solid 2px';
      }
      presetColorList[i].addEventListener('click', () => {
        for (let j = 0; j < presetColorList.length; j++) {
          (presetColorList[j] as HTMLElement).style.border = '#a9a9a9 solid 1px';
          (presetColorList[i] as HTMLElement).style.border = '#0090ff solid 2px';
        }
      })
    }
  ...
  public addPresetColors() {
    let presetColors = [];
    presetColors.push(this.project.article.contents.theme.colors);
    presetColors.push(['#000000', '#4C4C4C', '#a1a1a1', '#dfdfdf', '#ffffff']);
    this.cpPresetColors = _.flattenDeep(presetColors);
  }
  ...
  public setColorFromString(value: string, emit: boolean = true, update: boolean = true): void {
    ...
    if (emit || update) {
      // this.directiveInstance.themeLiskClickHandle(value);
    }
  ...
  private updateColorPicker(emit: boolean = true, update: boolean = true): void {
    if (this.sliderDimMax) {
      ...
      if (update) {
        ...
        const hexStr = this.service.rgbaToHex(rgba, allowHex8);
        // 去掉‘#’
        this.hexText = hexStr.slice(1, hexStr.length);
      }
      ...
      this.selectedDefaultColor = this.service.outputFormat(this.hsva, null, null);

      this.slider = new SliderPosition(
        (this.sliderH || this.hsva.h) * this.sliderDimMax.h - 2,
  ...
  private setDialogPosition(): void {
    if (this.cpDialogDisplay === 'inline') {
      ...
      const targetElementTop = this.directiveElementRef.nativeElement.getBoundingClientRect().top;
      if (this.useRootViewContainer || (position === 'fixed' && !parentNode)) {
        if (targetElementTop + this.dialogElement.nativeElement.offsetHeight + 40 > window.innerHeight) {
          this.dialogElement.nativeElement.style.top = 'auto';
          this.dialogElement.nativeElement.style.bottom = 0;
        } else {
          this.top = boxDirective.top;
          this.dialogElement.nativeElement.style.bottom = 'auto';
        }
  ...
  private showColorPicker() {
    this.showColorSlider = !this.showColorSlider;
  }
}
