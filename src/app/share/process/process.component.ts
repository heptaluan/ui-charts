  ...
  template: `
    <div class="d-flex flex-row align-items-center slider-container">
      ...
      <div class="progress-wrap" #slider>
        ...
        type="range" class="input-progress"
        [min]="min" [max]="max" [step]="step" [value]="getProgress()"
...
export class ProcessComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.projectType === 'infographic') {
      this.mySubscription
        .add(this._store.select(fromRoot.getCurrentProjectArticle).subscribe(res => {
          this.updateProgress()
      ...
      this.mySubscription
        .add(this._store.select(fromRoot.getCurrentChartArticle).subscribe(res => {
          this.updateProgress()
          this.animationButton = $('.block-container.show .animation-button')
          if (this.animationButton) {
            this.animationButton.removeClass('active')
