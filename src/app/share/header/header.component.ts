...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  allClick() {
    ...
    if (
      e.target !== this._el.nativeElement.querySelector('.no-click-area') &&
      this._el.nativeElement.querySelector('.user-dropdown') &&
      !this._el.nativeElement.querySelector('.user-dropdown').contains(e.target)
    ) {
  ...
  register() {
    ...
    if (window.location.href.indexOf('hmsr') > -1) {
      window.location.href =
        ...
        }&redirect=` + encodeURIComponent(`${window.location.href.split('#')[0]}#/pages/home/index`)
    } else {
      // window.location.href = `${this._api.getOldUrl()}/vis/auth/signin.html?redirect=` + encodeURIComponent(`${window.location.href.split('#')[0]}#/pages/home/index`);
      window.location.href =
        `${this._api.getOldUrl()}/vis/auth/signin.html?redirect=` +
