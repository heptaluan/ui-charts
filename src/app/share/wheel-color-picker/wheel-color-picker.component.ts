...
export class WheelColorPickerComponent implements OnInit, OnChanges {
  ...
  setColorSeleced(color?) {
    if (color) {
      ...
      if (
        ...
        (this.colorSelecedText.length === 3 || this.colorSelecedText.length === 6)
      ) {
        this.colorShowSeleced = '#' + this.colorSelecedText

        this.isChangeWCP = false
        $('#color-block').wheelColorPicker('setValue', '#' + this.colorSelecedText)
  ...
  setColorRgbaSeleced(color: string) {
    ...
    this.colorShowSeleced =
      '#' +
      parseInt(rgba[0]).toString(16).padStart(2, '0') +
      parseInt(rgba[1]).toString(16).padStart(2, '0') +
      parseInt(rgba[2]).toString(16).padStart(2, '0')
    this.colorSelecedText = this.colorShowSeleced.substring(1)

    this.opacitySeleced = parseFloat(rgba[3])
    this.colorOpacitySeleced = parseFloat(rgba[3]) * 100 + ''
    if (this.slider) {
      this.scursorLeft = (this.slider.nativeElement.offsetWidth / 100) * parseInt(this.colorOpacitySeleced)
    }
    if ($('#color-block')) {
      $('#color-block').wheelColorPicker('setValue', this.colorShowSeleced)
    }
  ...
  setRetrunColor() {
    ...
    if (this.isOpacityShow) {
      return (
        this.colorShowSeleced +
        Math.round(this.opacitySeleced * 255)
          .toString(16)
          .padStart(2, '0')
      )
    } else {
  ...
  setGradient() {
    ...
    this.gradientColorShow = `linear-gradient(${
      Number(this.gradientColor[length - 1].split(':')[1]) + 90
    }deg, ${colorPos})`
  ...
  addSlider(e) {
    ...
    if (lessIndex === null) {
      this.gradientColor.splice(
        this.gradientColor.length - 1,
        0,
        this.gradientColor[moreIndex].split(':')[0] + ':' + left
      )
    } else if (moreIndex === null) {
      this.gradientColor.splice(
        this.gradientColor.length - 1,
        0,
        this.gradientColor[lessIndex].split(':')[0] + ':' + left
      )
    } else {
  ...
  getBetweenGradientColor(lessIndex, moreIndex, pos) {
    ...
    newColor +=
      RoundNum(lessColorRGbArr[0] + (moreColorRGbArr[0] - lessColorRGbArr[0]) * left, 0) +
      ',' +
      RoundNum(lessColorRGbArr[1] + (moreColorRGbArr[1] - lessColorRGbArr[1]) * left, 0) +
      ',' +
      RoundNum(lessColorRGbArr[2] + (moreColorRGbArr[2] - lessColorRGbArr[2]) * left, 0) +
      ',1)'
  ...
  addRotateChangeEvent(e) {
    let that = this
    let target =
      e.target.className === 'angle-wrapper'
        ? $(e.target).find('.circle')
        : $(e.target).parents('.angle-wrapper').find('.circle')
