...
export class MagicBoxComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.isSelected && !changes.isSelected.isFirstChange()) {
      if (this.isProject && this._router.snapshot.routeConfig.path !== 'download') {
        // 判断 block 是否选中
        if (this.isSelected && !changes.isSelected.isFirstChange()) {
          ...
          $(this.parentElement).find('.line-item').show()
        } else {
          // this.parentElement.style.border = "1px solid transparent";
          $(this.el.nativeElement).hide()
          // 剔除直线 && 箭头的拖拽把手
          $(this.parentElement).find('.line-item').hide()
        }
      }
    }

    if (this.oldRatio !== 'initValue' && this.block.props.size.ratio !== this.oldRatio) {
      this.oldRatio = this.block.props.size.ratio
  ...
  setHandle() {
    if (this.block.type === 'chart') {
      $(this.parentElement).parent('.block-container').draggable('option', 'handle', '.echart-container')
    } else if (this.block.type === 'text') {
      $(this.parentElement).parent('.block-container').draggable('option', 'handle', '.block-text')
    }
  }
    ...
    if (newBlock.type === 'shape') {
      if (newBlock.shapeType === 'rectangle' || newBlock.shapeType === 'round-rectangle') {
        ...
        if (options.width) {
          ...
          newBlock.path = `${newBlock.props.size.width / 2} 0, 0 ${newBlock.props.size.height}, ${
            newBlock.props.size.width
  ...
  draggable() {
    $(this.parentElement)
      ...
      .draggable({
        ...
        drag: (event, ui) => {
          if (this.isSelected) {
            $(this.parentElement).find($('.topline')).css('display', 'block')
            $(this.parentElement).find($('.rightline')).css('display', 'block')
            $(this.parentElement).find($('.botline')).css('display', 'block')
            $(this.parentElement).find($('.leftline')).css('display', 'block')
          }
        ...
        stop: (event, ui) => {
          ...
          $(this.parentElement).find($('.topline')).css('display', 'none')
          $(this.parentElement).find($('.rightline')).css('display', 'none')
          $(this.parentElement).find($('.botline')).css('display', 'none')
          $(this.parentElement).find($('.leftline')).css('display', 'none')
        },
  ...
  transformable() {
    $(this.parentElement)
      ...
      .resizable({
        ...
        aspectRatio: this.block.props.size.ratio ? 1 / this.block.props.size.ratio : false,
        resize: (event, ui) => {
          // 缩放的时候 drag 防抖动
          ui.position.left = ui.position.left / this.scale
          ui.position.top = ui.position.top / this.scale
          ...
          switch (this.block.shapeType) {
            ...
            case 'triangle':
              svgEl.attr(
                'points',
                `${(ui.size.width + 2) / 2} 0, 0 ${ui.size.height + 2}, ${ui.size.width + 2} ${ui.size.height + 2}`
              )
              break
            case 'pentagon':
              svgEl.attr(
                'points',
                `${(ui.size.width + 2) / 2} 0, ${ui.size.width + 2} ${(ui.size.height + 2) * 0.4}, ${
                  (ui.size.width + 2) * 0.85
                } ${ui.size.height + 2}, ${(ui.size.width + 2) * 0.15} ${ui.size.height + 2}, 0 ${
                  (ui.size.height + 2) * 0.4
                }`
              )
            default:
        ...
        stop: (event, ui) => {
          ...
          if (this.isSelected) {
            ...
            $(this.el.nativeElement.parentElement).parent('.block-container').css('left', currentLeft)
            $(this.el.nativeElement.parentElement).parent('.block-container').css('top', currentTop)
